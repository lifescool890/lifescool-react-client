      ...
      >
        <Form.Item label="Set trending" name="trendings">
          {coursePoints.map((item, i) => {
            return (
              <Select
                className="b5-trend-select"
                key={item.type}
                onChange={(key, value) => handlePoints(key, value)}
              >
                {courses.map((item) => (
        ...
        <Form.Item>
          <Button htmlType="submit" className="td-cancel-button">
            Cancel
          </Button>
          <Button htmlType="submit" className="td-submit-button">
            Submit
          </Button>
        </Form.Item>
      </Form>
